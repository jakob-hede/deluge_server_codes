...
# from .logge import DelugapiStatusorLoggor
...
# from delugapi import DelugApi
from executin.logge import StatusorLoggor


class Statusor:
    def __init__(self):
        super().__init__()
        self.logger = StatusorLoggor()
    ...
    #     self.logger = DelugapiStatusorLoggor()
    ...
    def process_event(self, event_name):
        self.logger.info(f'Statusor process_event "{event_name}" called')
        ...
        def reactize() -> Generator[Any, Any, dict]:
            self.logger.debug('reactize')
            transaction = DelugApiStatusTransaction()
            reply = yield api_client.api.transactize(transaction)
            # Wait here for the transaction to complete
            transaction_response = transaction.response
            self.logger.debug(f"transaction_response: {transaction_response}")

            torrents_status_dict: dict = transaction_response.result
            self.logger.info(f'Total torrents: {len(torrents_status_dict)}')
            for indx, (torrent_id, torrent_dict) in enumerate(torrents_status_dict.items()):
                name = torrent_dict.get("name", "")
                display_name = name if len(name) <= 32 else name[:29] + '...'
                label = torrent_dict.get("label", "")
        ...
        def on_complete(result):
            self.logger.info(f'reactize completed with result: {str(result):.40}')
            # reactor.stop()

        def on_error(failure):
            self.logger.error(f'reactize failed: {failure}')
            # reactor.stop()
        ...
        self.logger.debug(f'Statusor process_event "{event_name}" completed wrap')
